    ...
    while (select) {
        ...
        switch (select) {
            case 0:
                ...
                break;
            case 1:
                if (Register(USER_SIZE)==-1) {
                    printf("注册错误");
                }
                break;
            case 2:
                value = Login(USER_SIZE);
                if (value == -1) {
                    printf("\n登录错误\n");
                }
                break;
            ...
            while (select_2) {
                ...
                switch (select_2) {
                    ...
                    case 5:
                        printf("输入学号:");
                        scanf("%ld", &num);
                        if (Scan(num, STU_SIZE) == -1) {
                            printf("\n查找错误\n");
                        }
                        break;
                    case 6:
                        if (Check(USER_SIZE, value) == -1) {
                            printf("\n没有权限\n");
                            break;
                        }
                        if (Revise(STU_SIZE) == -1) {
                            printf("\n修改错误\n");
                        }
                        break;
                    case 7:
